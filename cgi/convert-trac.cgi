...
re_CamelCaseUri = re.compile(r'(\s[^"\/\!\[\]\|]?)(([A-Z][a-z0-9]+){2,})')
re_NoUri = re.compile(r'\!(([A-Z][a-z0-9]+){2,})')
re_LabBook = re.compile(r'LabBook([A-Z][a-z]{2})([a-z]+)?(2[0-9]{3})')
...
def format_text(text):
    ...
    text = re_LabBook.sub(r'journal-\1-\3', text)
...
with open('template.htm') as f:
	...
	for line in f:
		template += line
print template.replace('%OUTPUT%', format_text(text))
